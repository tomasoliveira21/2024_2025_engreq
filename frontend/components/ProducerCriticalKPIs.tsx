...
const ProducerCriticalKPIs = ({ sessionToken }: KPIsProps) => {
  ...
  useEffect(() => {
    const fetchKPIs = async () => {
      try {
        setIsLoading(true);
        const data = await fetchProducerCriticalKPIs(sessionToken);
        setKpiData(data);
      } catch (error) {
        ...
        >
          ...
          <ul className="list-none space-y-4 text-gray-300">
            {seasonData.season.kpis.map((kpi, kpiIndex) => (
              ...
              >
                <p className="text-sm">{kpi.producer.businessName || "Unknown Producer"}</p>
                <p className="text-sm">
                  <span className="font-bold">Total Orders:</span> {kpi.totalOrders}
                </p>
                <p className="text-sm">
                  <span className="font-bold">Total Value:</span> {kpi.totalValue.toFixed(2)}€
                </p>
                <p className="text-sm">
                ...
                <p className="text-sm">
                  <span className="font-bold">Average Price:</span> {kpi.averagePrice.toFixed(2)}€
                </p>
                <p className="text-sm">{kpi.user.name} ({kpi.user.email})</p>
              </li>
            ))}
          </ul>
        </div>
...
export default ProducerCriticalKPIs;
